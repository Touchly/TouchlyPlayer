    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                float2 depth_uv = float2(o.uv.x*0.5 , o.uv.y);
                ...
                float ds1 = rgb2hue(tex2Dlod(_MainTex, float4(depth_uv + float2(-eps, 0.0), 0.0, 0.0)));
                float ds2 = rgb2hue(tex2Dlod(_MainTex, float4(depth_uv + float2(eps, 0.0), 0.0, 0.0)));
                float ds3 = rgb2hue(tex2Dlod(_MainTex, float4(depth_uv + float2(0.0, eps), 0.0, 0.0)));
                float ds4 = rgb2hue(tex2Dlod(_MainTex, float4(depth_uv + float2(0.0, -eps), 0.0, 0.0)));

                float ds5 = rgb2hue(tex2Dlod(_MainTex, float4(depth_uv + float2(-eps2, 0.0), 0.0, 0.0)));
                float ds6 = rgb2hue(tex2Dlod(_MainTex, float4(depth_uv + float2(eps2, 0.0), 0.0, 0.0)));
                float ds7 = rgb2hue(tex2Dlod(_MainTex, float4(depth_uv + float2(0.0, eps2), 0.0, 0.0)));
                float ds8 = rgb2hue(tex2Dlod(_MainTex, float4(depth_uv + float2(0.0, -eps2), 0.0, 0.0)));
                ...
                float3 pos = v.vertex + scale  * float3( s * (_Scale.x* v.vertex.x + _Scale.z), s * (_Scale.y* v.vertex.y + _Scale.w) , - s);
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float4 col;
                float3 rgb;
                float2 depth_uv = float2(i.uv.x*0.5 , i.uv.y);
                
                
                col = tex2D(_MainTex, uv);
                rgb = col.rgb;
                float sam = rgb2hue(tex2Dlod(_MainTex,float4(depth_uv,0,0)));
