namespace NatML.Vision {
    ...
    public sealed partial class SBSNetPredictor {
        ...
        public readonly struct Matte {
            ...
            public void Render (RenderTexture destination) {
                ...
                // Create buffer
                using var mapBuffer = new ComputeBuffer(2 * width * height, 2 * sizeof(float));
                // Upload
                ...
                tempBuffer.Create();
                // Render
                renderer = renderer ?? (ComputeShader)Resources.Load(@"ComputeShaders/SBSRenderer");
                renderer.SetBuffer(0, @"Map", mapBuffer);
                
                renderer.SetFloat(@"Max", max);
                renderer.SetFloat(@"Min", min);
                ...
                // Blit to destination
                Graphics.Blit(tempBuffer, destination, new Vector2(0.5f,0.5f), new Vector2(0, 0));
